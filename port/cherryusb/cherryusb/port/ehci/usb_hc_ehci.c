...
static void ehci_urb_waitup(struct usbh_bus *bus, struct usbh_urb *urb)
{
    ...
    /* Local addition (upstream v1.6.1): on dcache-enabled hosts the EHCI
     * DMA wrote IN buffers behind the CPU's back; upstream assumes
     * cache-less MCUs or .noncacheable buffers.  Drop stale lines before
     * any waiter or completion callback reads the data.  Note: do not
     * gate on urb->ep direction — control EP0 transfers carry the
     * direction in setup[0], and ep0's address bit7 is always 0. */
#ifdef CONFIG_USB_DCACHE_ENABLE
    if (urb->errorcode >= 0 && urb->transfer_buffer && urb->transfer_buffer_length) {
...
/* Local addition (upstream v1.6.1): called once at the end of usb_hc_init,
 * after RUN/CONFIGFLAG/port power are up.  Platforms with devices already
 * plugged at reset use it to force a port status refresh (no connect-change
 * edge is generated for pre-inserted devices). */
__WEAK void usb_hc_ehci_post_init(struct usbh_bus *bus)
{
    (void)bus;
}
