...
/* Embox port: the section name must stay a C identifier so GNU ld provides
 * the __start_/__stop_ symbols without a custom linker script; the live
 * reference from usbh_initialize roots the section under --gc-sections. */
#define CLASS_INFO_DEFINE __attribute__((section("usbh_class_info"), used)) __ALIGNED(1)
